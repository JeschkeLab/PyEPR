...
class Interface:
    ...
    def tune_RectPulse(self,freq, B, reptime, tp, offset_freq, shots=200, verbosity=0,**kwargs):
        """
        ...
        offset_freq : float
            The offset frequency of the rectangular pulse in GHz. This is used to shift the pulse away from the central frequency of the experiment.
        shots : int, optional
        ...
        ** Interface specific launch commands
        ...
        p180 = pulses.RectPulse(tp=tp,freq=offset_freq,flipangle=np.pi,scale=amp)
    ...
    def tune_GaussianPulse(self,freq, B, reptime, tp, offset_freq, shots=200, verbosity=0,**kwargs):
        """
        ...
        tp : float
            ...
            This will be rounded to the nearest 2*waveform_precision to ensure that the pi pulse is of the correct length.
        offset_freq : float
            The offset frequency of the Gaussian pulse in GHz. This is used to shift the pulse away from the central frequency of the experiment.
        shots : int, optional
            The number of shots to be used for the frequency sweep, by default 200.
        verbosity : int, optional
            The verbosity level, by default 0. If > 0, the scale values will be printed.
        ...
        ** Interface specific launch commands
        ...
        p180 = pulses.GaussianPulse(tp=tp,freq=offset_freq,flipangle=np.pi,scale=amp)
    ...
    def tune_KBBPulse(self,*, freq, B, reptime, tp, init_freq, final_freq, shots=200, verbosity=0,**kwargs):
        ...
        ** Interface specific launch commands
        ...
        dim = kwargs.pop('dim', 50)
        start = kwargs.pop('start', 0.0)
        amp = Parameter(name="Amp", value=start, step=step, dim=dim)

        p90 = pulses.ChirpPulse(
            tp=tp,init_freq=init_freq,final_freq=final_freq,flipangle=np.pi/2)
        p180 = pulses.ChirpPulse(
            tp=tp*0.5,init_freq=init_freq,final_freq=final_freq,flipangle=np.pi)
